...
#method that mutates an array according to its elements that have key/value pairs in the given hash 
def ele_replace!(arr, hash)
    arr.each_with_index { |el, idx| arr[idx] = hash[el] if hash.has_key?(el) } 
    arr 
end 
