...
describe Dessert do
  let(:chef) { double("chef", :name => 'jake') }
  ...
  describe "#add_ingredient" do
    it "adds an ingredient to the ingredients array" do 
      expect(dessert.ingredients).to_not include('icing')
  ...
  describe "#mix!" do
    it "shuffles the ingredient array" do 
      ingredients = ['icing', 'sprinkles', 'fudge']
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do 
      allow(chef).to receive(:titleize).and_return("Chef Jake the Great Baker")
      expect(dessert.serve).to eq("Chef Jake the Great Baker has made 10 brownies!")
    end 
  end
