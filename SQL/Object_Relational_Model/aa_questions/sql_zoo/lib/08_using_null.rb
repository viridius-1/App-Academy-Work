...
def all_teachers_join
  ...
  execute(<<-SQL)
    SELECT 
      teachers.name AS teacher, depts.name AS dept 
    FROM 
      teachers 
    LEFT JOIN 
...
def teachers_and_depts
  ...
  execute(<<-SQL)
    SELECT 
      teachers.name AS teacher, COALESCE(depts.name, 'None') AS dept 
    FROM 
      teachers 
    LEFT JOIN 
      depts ON teachers.dept_id = depts.id
...
def num_teachers_and_mobiles
  ...
  execute(<<-SQL)
    SELECT 
      COUNT(name) AS teachers, COUNT(mobile) AS phones 
    FROM 
      teachers
  SQL
end
...
def dept_staff_counts
  ...
  execute(<<-SQL)
    SELECT 
      depts.name AS dept, COUNT(teachers.name) AS staff 
    FROM 
      teachers 
    RIGHT JOIN 
      depts ON teachers.dept_id = depts.id 
