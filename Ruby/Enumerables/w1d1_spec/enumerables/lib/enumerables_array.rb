...
def subwords(word, dictionary)
  substrings_of_word = substrings(word) 
  substrings_of_word.select { |substring| dictionary.include?(substring) }.uniq 
end
...
class Array
  def my_map(&prc)
    self.my_each.map { |el| prc.call(el) }
  end

  def my_select(&prc)
    self.my_each.select { |el| prc.call(el) }
  end
