...
def harrison_ford
  ...
  Movie.select('distinct movies.id', :title).joins(:actors).joins(:castings).where("name = 'Harrison Ford' AND castings.ord != 1")
end
...
def directed_by_one_of(them)
  ...
  Actor.joins(:directed_movies).select('movies.id', 'movies.title').where(name: them)
end
